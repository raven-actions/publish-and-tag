...
test('prints help when no arguments are passed', async () => {
  ...
  expect(logger.log).toHaveBeenCalledWith(expect.stringMatching(/Usage: npx actions-toolkit <name>/))
})
...
test('fails to start creating project in a directory that already exists', async () => {
  ...
  expect(logger.fatal).toHaveBeenCalledWith(expect.stringMatching(/Folder .*tests\/fixtures\/workspaces\/action-already-exists already exists!/))
})
...
test('fails to start when mkdir fails for an unknown reason', async () => {
  ...
  await expect(runCLI('__tmp/my-project-name')).rejects.toThrowError('KABOOM')
})
...
test('creates project with labels passed to Dockerfile from questionnaire', async () => {
  ...
  const readGeneratedFile = (name) => fs.readFileSync(path.resolve(__dirname, '../__tmp/my-project-name', name), 'utf-8')

  await runCLI('__tmp/my-project-name')

  expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/Creating folder .*my-project-name.../))
  expect(logger.star).toHaveBeenCalledWith(expect.stringMatching(/Welcome to actions-toolkit/))
  expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/Creating package.json/))
  expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/Creating Dockerfile/))
  expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/Creating index.js/))
  expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/Creating index.test.js/))
